...
\textbf{Grupo} nr. & 90
\\\hline
a89533 & Ana Luisa Carneiro
\\
a89506 & Ana Rita Peixoto
\\
a89612 & Luis Miguel Pinto
\end{tabular}
\end{center}
...
No anexo \ref{sec:codigo} é dada uma implementação da operação de permuta aleatória de uma lista que pode ser útil para resolver este exercício.
    ...
    \begin{figure}\centering
    \includegraphics[scale=0.20]{images/Ladrilho.png}
    \caption{Mosaico de Truchet-Smith gerado no problema 5.}
    \label{fig:Ladrilho}
    \end{figure}
...
discollect [] = []
discollect (k:kx) = separate k ++ discollect kx where
                    separate (b, []) = []
                    separate (b, (h:t)) = (b,h) : separate (b, t)
...
tar = cataExp g where
  g = (either g1 g2)
    where g1 v = [([],v)]
          g2 (s, []) = []
          g2 (s,(h:t)) = (putSFirst s h) ++ g2(s,t)

putSFirst :: String -> [(String, String)] -> [(String,String)]
putSFirst s [] = []
putSFirst s ((h,t):f)= [(s++h,t)] ++ putSFirst s f

dic_rd :: String -> Dict -> Maybe [String]
dic_rd p d = read_dic p (dic_exp d)

read_dic :: String -> [(String,[String])] -> Maybe [String] 
read_dic p [] = Nothing 
read_dic p ((t,h:x):g) = if(p==t) then Just (h:x) else read_dic p g

dic_in :: String -> String -> Dict -> Dict
dic_in p s d = dic_imp(insere_dic p s (dic_exp d))

insere_dic :: String -> String -> [(String,[String])] -> [(String,[String])]
insere_dic p s [] = [(p,[s])]
insere_dic p s ((t,h):g) = if(p==t)then ((t,aux s h):g) else (t,h):insere_dic p s g
                        where aux s [] = [s]
                              aux s (h:t) = if(s==h) then h:t else h:(aux s t)
...
maisDir = cataBTree g
  where g = (either g1 g2)
          where g1 () = Nothing
                g2 (n,(_,Nothing)) = Just n
                g2 (n,t) = p2 t

maisEsq = cataBTree g
  where g = (either g1 g2)
          where g1 () = Nothing 
                g2 (n,(Nothing,_)) = Just n
                g2 (n,t) = p1 t

insOrd' x = cataBTree g 
  where g = (either g1 g2)
          where g1 () = split (const(Node(x,(Empty ,Empty)))) (const Empty) ()
                g2 (a,(e,d)) = split (const h2) (const k2) (a,(e,d))
                             where h2 = if(x>a) then Node(a,(p2 e, p1 d)) else Node(a,(p1 e, p2 d))
                                   k2 = Node(a,(p2 e, p2 d))

insOrd x b = p1 (insOrd' x b)

isOrd' = cataBTree g where 
  g = (either g1 g2)
    where g1 () = split (const True) (const Empty) ()
          g2 a = split (verifica) (nodo) a

nodo :: Ord a => (a,((Bool,BTree a),(Bool,BTree a))) -> BTree a
nodo (a,((b,c),(d,e))) = Node(a,(c,e))

verifica :: Ord a => (a,((Bool,BTree a),(Bool,BTree a))) -> Bool
verifica (a,((b,c),(d,e))) = (verEsq a c) && (verDir a e) && b && d

verEsq :: Ord a => a -> BTree a -> Bool
verEsq a Empty = True
verEsq a (Node(b,(t1,t2))) = (a >= b) && (verEsq b t1) && (verDir b t2)

verDir :: Ord a => a -> BTree a -> Bool
verDir a Empty = True
verDir a (Node(b,(t1,t2))) = (a <= b) && (verEsq b t1) && (verDir b t2)

isOrd = p1 . isOrd'

rrot Empty = Empty
rrot (Node(a,(Empty,d))) = Node(a,(Empty,d))
rrot (Node(r,(Node(l,(le,ld)),d))) = Node(l,(le,Node(r,(ld, d))))

lrot Empty = Empty
lrot (Node(a,(e,Empty))) = Node(a,(e,Empty))
lrot (Node(r,(l,Node(d,(de,dd))))) = Node(d,(Node(r,(l,de)),dd))
...
splay' = cataBTree g
      where g = (either g1 g2) where 
                  g1 () _= Empty
                  g2 (a,(e,d)) [] = Node(a,(e [], d []))  
                  g2 (a,(e,d)) (h:t) = if(h == False) then lrot(Node(a,(e [],d t))) else rrot(Node(a,(e t, d [])))
...
navLTree = cataLTree g where 
          g = (either g1 g2)
            where g1 x _= Leaf x
                  g2 (t,f) [] = Fork(t [], f [])  
                  g2 (a,b) (h:t) = if(h == False) then (b t) else (a t) 

\end{code}
Diagrama do anamorfismo de Bdt:
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |Bdt A|
&
...
bnavLTree = cataLTree g
  where g = (either g1 g2)
          where g1 x _ = Leaf x
                ...
                g2 (l,r) (Node(a,(e,d))) = if(a) then (l e) else (r e) 


pbnavLTree = cataLTree g
  where g = (either g1 g2)
          where g1 a _ = mkD [(Leaf a,1)]
                g2 (l,r) Empty = (Probability.cond (mkD [(False,0.5),(True,0.5)]) (l Empty) (r Empty))
                g2 (l,r) (Node(a,(e,d))) = (Probability.cond a (l e) (r e))
...
ladrilhos = do{n <- getStdRandom(randomR (0, linhas*colunas)); f n}
            where f n = permuta((rep n 1)++(rep ((linhas*colunas)-n) 2))
                  linhas = 10
                  colunas = 10

lad :: Float -> Float -> IO[Integer] -> IO[Picture]
lad x y m = do {lista <- m; return (f x y lista)}
            where f x y [] = []
                  f x y (h:t) = let azulejo = if(h == 1) then truchet1 else truchet2
                                    colunas = 10 in 
                                if (x /= ((-400) + (colunas-1)*80)) then ((Translate (x) (y) azulejo) : f (x+80) (y) t) 
                                else ((Translate (x) (y) azulejo) : f (x-(80*(colunas-1))) (y-80) t)

convert :: IO[Picture] -> IO Picture
convert m = do{lista <- m; return(Pictures lista)}

main :: IO ()
main = do{pic <- (convert (lad (-400) 320 ladrilhos)); display janela white pic}                                         
